...
export default class MultiSelectCombobox extends LightningElement {
    ...
    //todisplay profile picklist values
    showOptions() {
        if (this.disabled == false && (this.options || this.objects)) {
            this.message = '';
            this.searchString = '';
    ...
    removePill(event) {
        ...
        var count = 0;
        this.values = this.values.filter(va => va != value);
        this.passDatatoParent();
        var options = JSON.parse(JSON.stringify(this.optionData));
        for (var i = 0; i < options.length; i++) {
            if (options[i].value === value) {
                options[i].selected = false;
                options.splice(options.indexOf(options[i].value), 1);
            }
            if (options[i].selected) {
                count++;
            }
    ...
    blurEvent() {
        ...
        for (var i = 0; i < this.optionData.length; i++) {
            ...
            if (this.optionData[i].selected) {
                count++;
                let data = this.optionData[i].value;
                this.selectedData = this.selectedData + ',' + data;
            }
        }
       // console.log('blur values==>',JSON.stringify(this.values));
        if (this.multiSelect) {
            this.searchString = count + ' Option(s) Selected';
        }
        else {
